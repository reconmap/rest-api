...
INSERT INTO task (project_id, name, description, command, command_parser)
VALUES (1, 'Run port scanner', 'Use nmap to detect all open ports', 'nmap -oX out.xml -v -sS @@TARGET@@', 'nmap'),
       (1, 'Run SQL injection scanner', 'Use sqlmap to test the application for SQL injection vulnerabilities',
        'python sqlmap.py -u @@TARGET@@ --method POST --data "username=foo&password=bar" -p username --level 5 --dbms=mysql -v 1 --tables',
        'sqlmap'),
       (1, 'Check domain expiration date', 'Use whois or other tools to check when the domain expiration is.',
        'whois @@TARGET@@', NULL),
       (2, 'Run port scanner', 'Use nmap to detect all open ports', 'nmap -oX out.xml -v -sS @@TARGET@@', 'nmap'),
       (2, 'Run SQL injection scanner', 'Use sqlmap to test the application for SQL injection vulnerabilities',
        'python sqlmap.py -u @@TARGET@@ --method POST --data "username=foo&password=bar" -p username --level 5 --dbms=mysql -v 1 --tables',
        'sqlmap'),
